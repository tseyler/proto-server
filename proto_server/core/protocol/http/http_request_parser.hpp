	...
	namespace protocol
	{
		namespace http
		{
			...
			public:
				...
				virtual http_parse_result protocol_parse(const proto_net_data &formed, http_request_message &parsed);

				// this forms a well-formed message.  Returns zero if successful
				virtual http_parse_result protocol_form(const http_request_message &parsed, proto_net_data &formed);
