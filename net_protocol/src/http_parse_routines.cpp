...
size_t 
proto_http::find_crlf(const std::string& str, 
		      size_t pos)
{
    return str.find(proto_http::crlf, pos);
}

size_t 
proto_http::find_line(const std::string& str, 
...
size_t 
proto_http::get_lines(const std::string& http, 
		      lines_t& lines)
{
    size_t pos(0);
    size_t count(0);
...
size_t 
proto_http::get_header_field_lines(const lines_t& http_lines, 
...
size_t 
proto_http::message_body_position(const net_data& data)
{
    size_t pos(0);
    size_t cr_found(0);
    size_t lf_found(0);
...
size_t 
proto_http::tokenize_line(const std::string line,
			  matches_t& tokens)
{
    std::string sep(" \t");
...
size_t 
proto_http::tokenize_header_field(const std::string line,
				  matches_t& tokens)
{
    std::string sep(":");
    split(line, sep, tokens);
...
proto_http::validate_request_method(const std::string method)
{
    return search_pattern(method, method_pat) ? http_parse_success : http_request_method_error; 
}

proto_http::http_parse_result 
proto_http::validate_request_uri(const std::string& uri, 
...
proto_http::http_parse_result 
proto_http::validate_http_version(const std::string& http_version)
{
    return search_pattern(http_version, http_ver_pat) ? http_parse_success : http_version_error; 
}

proto_http::http_parse_result 
proto_http::validate_request_line(matches_t& tokens)
{
...
proto_http::http_parse_result
proto_http::validate_header_field(net_proto::matches_t& tokens, 
				  const std::string& field_name_pat)
{
    http_parse_result res(http_parse_success);

    if (tokens.size() == 2)
	res = search_pattern(tokens[0], field_name_pat) ? http_parse_success 
...
proto_http::http_parse_result 
proto_http::validate_header_fields(const lines_t& lines,
				   ...
				   http_header_fields& http_fields)
{
	...
	while (it != lines.end())
	{
	    ...
	    {
		http_parse_result line_res = validate_header_field(tokens, pat);
...
proto_http::http_parse_result
proto_http::validate_http_headers(const lines_t& lines,
				  http_headers& headers)
{
   http_parse_result res(http_parse_success);
   http_headers_map& hdr_map = headers.get_headers_map();
   for (http_headers_map_iterator it = hdr_map.begin();
	it != hdr_map.end();
	it++)
   {
...
proto_http::http_parse_result 
proto_http::validate_status_code(const std::string& status_code)
{
    return search_pattern(status_code, status_code_pat) ? http_parse_success 
	: http_status_code_error;
}

proto_http::http_parse_result 
proto_http::validate_status_line(matches_t& tokens)
{
    http_parse_result res(http_parse_success);
