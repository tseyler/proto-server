...
proto_http::http_status_code::http_status_code(int code /*= 400*/) : status_code_(code)
{ }

proto_http::http_status_code::http_status_code(const http_status_code& copy) : status_code_(400)
{
    clone(copy);
}

proto_http::http_status_code& 
proto_http::http_status_code::operator = (const http_status_code& rhs)
{
    if (&rhs != this)
	clone(rhs);
...
void 
proto_http::http_status_code::clone(const http_status_code& copy)
{
    status_code_ = copy.status_code();
}

int 
proto_http::http_status_code::status_code(void) const 
{ 
    return status_code_; 
}

void 
proto_http::http_status_code::status_code(int code) 
{ 
    status_code_ = code; 
}

proto_http::status_code_class 
proto_http::http_status_code::code_class(void) const
{
...
std::string 
proto_http::http_status_code::reason_phrase(void) const
{
    std::string phrase;
...
std::string 
proto_http::http_status_code::to_string(void) const
{
    std::stringstream ss;
