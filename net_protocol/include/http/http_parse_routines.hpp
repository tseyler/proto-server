...
#define HTTP_PARSE_SUCCEEDED( r ) (r == proto_http::http_parse_success)
...
namespace proto_http
{
    ...
    typedef enum { http_parse_success,
    ...
    size_t message_body_position(const net_data& data);

    // tokenize routines
    size_t tokenize_line(const std::string line,
			 net_proto::matches_t& tokens);
    size_t tokenize_header_field(const std::string line,
				 net_proto::matches_t& tokens);
    ...
    http_parse_result validate_request_uri(const std::string& uri, 
					   const std::string& method);
    // validate http version
    http_parse_result validate_http_version(const std::string& http_version);
    // validate request line
    http_parse_result validate_request_line(net_proto::matches_t& tokens);
    // validate header field
    http_parse_result validate_header_field(net_proto::matches_t& tokens, 
					    const std::string& field_name_pat);
    // validate header fields by category
    http_parse_result validate_header_fields(const lines_t& lines,
    ...
    http_parse_result validate_status_line(net_proto::matches_t& tokens);
}
